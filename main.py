...
def load_config():
    ...
    if config_path.exists():
        with config_path.open() as f:
            loaded = json.load(f)
        chain_data = {**dataclasses.asdict(DEFAULT_CONFIG.chain), **loaded.get("chain", {})}
        pool_data = {**dataclasses.asdict(DEFAULT_CONFIG.pool), **loaded.get("pool", {})}
        return DEFAULT_CONFIG.__class__(
            chain=DEFAULT_CONFIG.chain.__class__(**chain_data),
            pool=DEFAULT_CONFIG.pool.__class__(**pool_data),
            tokens=loaded.get("tokens", DEFAULT_CONFIG.tokens),
        )
